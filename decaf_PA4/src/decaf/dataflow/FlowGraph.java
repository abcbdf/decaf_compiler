        ...
        do {
            ...
            for (BasicBlock bb : bbs) {
                for (int i = 0; i < 2; i++) {
                    ...
                    bb.liveOutPair.addAll(bbs.get(bb.next[i]).liveInPair);
                }
                //System.out.println(bb.liveOutPair);
                //System.out.println(bb.bbNum + "def" + ":" + bb.def);
                bb.liveOut.removeAll(bb.def);
                bb.liveOutPair.removeIf(x -> bb.hasDefined.contains(x.tmp));
                if (bb.liveIn.addAll(bb.liveOut))
                    changed = true;
                if (bb.liveInPair.addAll(bb.liveOutPair))
                	changed = true;
                for (int i = 0; i < 2; i++) {
                    bb.liveOut.addAll(bbs.get(bb.next[i]).liveIn);
                    bb.liveOutPair.addAll(bbs.get(bb.next[i]).liveInPair);
                }
                //System.out.println(bb.bbNum + ":" + bb.liveOutPair);
            }
        } while (changed);
    }

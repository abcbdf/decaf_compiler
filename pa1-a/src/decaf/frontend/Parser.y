...
%token IF     ELSE        RETURN   BREAK   NEW	DO	OD
%token PRINT  READ_INTEGER         READ_LINE	DCOPY	SCOPY	CASE	DEFAULT	PRINTCOMP
%token LITERAL
%token IDENTIFIER	  AND    OR    STATIC  INSTANCEOF
%token LESS_EQUAL   GREATER_EQUAL  EQUAL   NOT_EQUAL	SEPARATOR
%token '+'  '-'  '*'  '/'  '%'  '='  '>'  '<'  '.'	':'
%token ','  ';'  '!'  '('  ')'  '['  ']'  '{'  '}'	'@'	'$'	'#'
...
Expr            :	LValue
                ...
                |	'$' Expr
                	{
                		$$.expr = new Tree.Unary(Tree.DOL, $2.expr, $1.loc);
                	}
                |	'#' Expr
                	{
                		$$.expr = new Tree.Unary(Tree.POUND, $2.expr, $1.loc);
                	}
                |	'!' Expr
                	{
                ...
                |	DCOPY '(' Expr ')'
                	{
                		$$.expr = new Tree.DcopyExpr($1.loc, $3.expr);
                	}
                |	SCOPY '(' Expr ')'
                	{
                		$$.expr = new Tree.ScopyExpr($1.loc, $3.expr);
                	}
                |	CASE '(' Expr ')' '{' ACaseList DefaultExpr '}'
                	{
                		$$.expr = new Tree.CaseExpr($1.loc, $3.expr, $6.elist, $7.expr);
                	}
                |	NEW IDENTIFIER '(' ')'
                	{
...
DoBranch		:	DoSubStmt SEPARATOR
					{
						$$.stmt = new Tree.DoBranch($1.loc, $1.stmt);
					}
				;
DoSubStmt		:	Expr ':' Stmt
					{
						$$.stmt = new Tree.DoSubStmt($1.loc, $1.expr, $3.stmt);
					}
				;
...
ACaseExpr		:	Constant ':' Expr ';'
					{
						$$.expr = new Tree.ACaseExpr($1.loc, $1.expr, $3.expr);
					}
				;

DefaultExpr		:	DEFAULT ':' Expr ';'
					{
						$$.expr = new Tree.DefaultExpr($1.loc, $3.expr);
					}	
				;			
...
DoStmt			:	DO DoBranchList DoSubStmt OD
					{
						$$.stmt = new Tree.DoStmt($1.loc, $2.slist, $3.stmt);
					}
				;

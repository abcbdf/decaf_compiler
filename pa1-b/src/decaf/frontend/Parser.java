...
public class Parser extends Table {
    ...
    private SemValue parse(int symbol, Set<Integer> follow) {
        ...
        if (result == null) {
        		error();
        		while (follow.contains(lookahead) == false && beginSet(symbol).contains(lookahead) == false && followSet(symbol).contains(lookahead) == false)
        			lookahead = lex();
        		if (beginSet(symbol).contains(lookahead) == true) {

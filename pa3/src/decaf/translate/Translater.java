...
public class Translater {
	...
	public Temp genDiv(Temp src1, Temp src2) {		
		...
		genBeqz(genEqu(src2, genLoadImm4(0)), exit);
		Temp msg = genLoadStrConst(RuntimeError.DEVISION_BY_ZERO);
		genParm(msg);
	...
	public Temp genComplexImm4(int imm) {
		Temp size = genLoadImm4(8);
		genParm(size);
		Temp dst = genIntrinsicCall(Intrinsic.ALLOCATE);
		Temp imag = genLoadImm4(imm);
	...
	public Temp genToComp(Temp src, Type type) {
		if (type.equal(BaseType.INT))
		{
			Temp size = genLoadImm4(8);
			genParm(size);
			Temp dst = genIntrinsicCall(Intrinsic.ALLOCATE);
		...
		else if (type.equal(BaseType.COMPLEX))
		{
			return src;
		}
		else
	...
	public Temp genAddComp(Temp src1, Temp src2) {
		Temp size = genLoadImm4(8);
		genParm(size);
		Temp dst = genIntrinsicCall(Intrinsic.ALLOCATE);
		Temp real = genAdd(genLoad(src1, 0), genLoad(src2, 0));
		Temp imag = genAdd(genLoad(src1, 4), genLoad(src2, 4));
		genStore(real, dst, 0);
		genStore(imag, dst, 4);
		return dst;
	}
	
	public Temp genMulComp(Temp src1, Temp src2) {
		Temp size = genLoadImm4(8);
		genParm(size);
		Temp dst = genIntrinsicCall(Intrinsic.ALLOCATE);
		Temp real = genSub(genMul(genLoad(src1, 0), genLoad(src2, 0)), genMul(genLoad(src1, 4), genLoad(src2, 4)));
		Temp imag = genAdd(genMul(genLoad(src1, 0), genLoad(src2, 4)), genMul(genLoad(src1, 4), genLoad(src2, 0)));
		genStore(real, dst, 0);
		genStore(imag, dst, 4);
		return dst;
	}

...
%token VOID   BOOL  INT   STRING   COMPLEX  CLASS CASE  DEFAULT
...
Expr            :	LValue
                ...
                |   CASE '(' Expr ')' '{' CasePairs DefCasePair '}'
                    {
                        $$.expr = new Tree.CaseExpr($3.expr, $6.pairlist, $7.defpair, $1.loc);
                    }
                ;
...
CasePairs       :   CasePairs CasePair
                ...
                |
                    {
                        $$.pairlist = new ArrayList<Tree.CasePair>();
                    }
                ;
CasePair        :   Constant ':' Expr ';'
                    {
                        $$.pair = new Tree.CasePair($1.expr, $3.expr, $1.loc);
                    }
                ;
DefCasePair     :   DEFAULT ':' Expr ';'
                    {
                        $$.defpair = new Tree.DefCasePair($3.expr, $1.loc);
                    }
                ;
...
DoStmt          :   DO Branches OD
                    {
                        $$.stmt = new Tree.Do($2.branches, $1.loc);
                    }
                ;
Branches        :   Branches SPLIT CondStmt
                    {
                        $$.branches.add((Tree.Cond)$3.stmt);
                    }
                |   CondStmt
                    {
                        $$.branches = new ArrayList<Tree.Cond>();
                        $$.branches.add((Tree.Cond)$1.stmt);
                    }
                ;
CondStmt        :   Expr ':' Stmt
                    {
                        $$.stmt = new Tree.Cond($1.expr, $3.stmt, $1.loc);
                    }
                ;
%%
